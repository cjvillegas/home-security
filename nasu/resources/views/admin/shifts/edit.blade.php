    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shifts.update", [$shift->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.shift.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $shift->name) }}">
                @if($errors->has('name'))
