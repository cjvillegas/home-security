...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orderhistories.update", [$orderhistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order_number') ? 'is-invalid' : '' }}" name="order_number_id" id="order_number_id">
                    @foreach($order_numbers as $id => $order_number)
                        <option value="{{ $id }}" {{ (old('order_number_id') ? old('order_number_id') : $orderhistory->order_number->id ?? '') == $id ? 'selected' : '' }}>{{ $order_number }}</option>
                    @endforeach
                </select>
